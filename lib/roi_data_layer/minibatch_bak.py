...
def _get_image_blob(roidb, scale_inds, crop_path=None):
  ...
  for i in range(num_images):
    ...
    # roidb[i] = gray_roi
    if gray_aug and crop_path is not None and roidb[i]['image'] is None:
      print('gray')
      import random
      im = np.ones((500, 500, 3), dtype=np.uint8)
      im = im * random.randint(0, 255)
      gt_boxes = np.array([])
      gt_boxes = np.empty((0, 5), dtype=np.float32)
      im, cord_list = sample_1.copy_paste_aug(im, crop_path=cfg.TRAIN.GRAY_PATH, gt_boxes=gt_boxes, save=True)  # True
    ...
    if crop_path is not None:
      ...
      if gray_aug and roidb[i]['image'] is None:
        im, cord_list = sample_1.copy_paste_aug(im, crop_path = cfg.TRAIN.GRAY_PATH, gt_boxes=gt_boxes, save=True, gray=True)  # True
      else:
        im, cord_list = sample_1.copy_paste_aug(im, crop_path, gt_boxes, save=True)#True
      if roidb[i]['flipped']:
        im = im[:, ::-1, :]
