...
def correlation_distillation_cosloss(fea1,fea2,fea1_old,fea2_old):
    ...
    for i in range(0, fea1.shape[0]):
        ...
        corr_loss += torch.norm((sim_matrix_org[i]-sim_matrix[i]).mul(sim_matrix_org[i]-sim_matrix[i]))
...
class _RPN_distil(nn.Module):
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes, fasterRCNN_org=None, base_feat_ori=None):
        ...
        if fasterRCNN_org:
            l1_loss_fn = torch.nn.L1Loss(reduce=True, size_average=True)
            ################# ori fasterrcnn #####################################################
            rpn_conv1_ori = F.relu(fasterRCNN_org.RCNN_rpn.RPN_Conv(base_feat_ori), inplace=True)
            # get rpn classification score
            rpn_cls_score_ori = fasterRCNN_org.RCNN_rpn.RPN_cls_score(rpn_conv1_ori)
            rpn_cls_score_max = rpn_cls_score*(rpn_cls_score>=rpn_cls_score_ori).float()+rpn_cls_score_ori*(rpn_cls_score<rpn_cls_score_ori).float()
            ...
            rpn_cls_prob_ori = fasterRCNN_org.RCNN_rpn.reshape(rpn_cls_prob_reshape_ori, self.nc_score_out)
            rpn_cls_prob_max = rpn_cls_prob*(rpn_cls_prob>=rpn_cls_prob_ori).float()+rpn_cls_prob_ori*(rpn_cls_prob<rpn_cls_prob_ori).float()
            # rpn_output_loss = l1_loss_fn(rpn_cls_prob, rpn_cls_prob_max)

            # rpn_cls_score_reshape_max = self.reshape(rpn_cls_score_max, 2)
            # rpn_cls_prob_reshape_max = F.softmax(rpn_cls_score_reshape_max, 1)
            # rpn_cls_prob_max = self.reshape(rpn_cls_prob_reshape_max, self.nc_score_out)

            fg_prob = rpn_cls_prob[:, len(self.anchor_scales) * len(self.anchor_ratios):, :, :]
            bg_prob = rpn_cls_prob[:, :len(self.anchor_scales) * len(self.anchor_ratios), :, :]
            fg_prob_ori = rpn_cls_prob_ori[:, len(self.anchor_scales) * len(self.anchor_ratios):, :, :]
            bg_prob_ori = rpn_cls_prob_ori[:, :len(self.anchor_scales) * len(self.anchor_ratios), :, :]
            rpn_cls_prob_max[:, len(self.anchor_scales) * len(self.anchor_ratios):, :, :] = fg_prob*(fg_prob>=fg_prob_ori).float()+fg_prob_ori*(fg_prob_ori>fg_prob).float()
            rpn_cls_prob_max[:, :len(self.anchor_scales) * len(self.anchor_ratios), :, :] = bg_prob*(bg_prob<=bg_prob_ori).float()+bg_prob_ori*(bg_prob_ori<bg_prob).float()

            fg_idx_ori = ((rpn_cls_prob_ori[:, len(self.anchor_scales) * len(self.anchor_ratios):, :, :] > 0.5)).nonzero()
            fg_fea_ori = rpn_conv1_ori[fg_idx_ori[:,0].long(),:,fg_idx_ori[:,2].long(),fg_idx_ori[:,3].long()]
            fg_fea = rpn_conv1[fg_idx_ori[:,0].long(),:,fg_idx_ori[:,2].long(),fg_idx_ori[:,3].long()]
            if fg_idx_ori.shape[0]!=0:
                rpn_output_loss = l1_loss_fn(fg_fea, fg_fea_ori)
            else:
                rpn_output_loss = torch.Tensor([0]).cuda()#l1_loss_fn(fg_fea, fg_fea_ori)
            # if torch.any(torch.isnan(rpn_output_loss)):
            #     print(fg_fea,fg_fea_ori)
            # from SupContrast.losses import SupConLoss
            # temp = 0.07
            # criterion = SupConLoss(temperature=temp, contrast_mode='all')  # all,one
            # # features: [bsz, n_views, f_dim]
            # # `n_views` is the number of crops from each image
            # # better be L2 normalized in f_dim dimension
            # fg_idx_ori=((rpn_cls_prob_max[:, len(self.anchor_scales) * len(self.anchor_ratios):, :, :]>0.5)).nonzero()#((rpn_cls_prob_max<0.9) & (rpn_cls_prob_max>0.5)).nonzero()
            # bg_idx_ori=((rpn_cls_prob_max[:, :len(self.anchor_scales) * len(self.anchor_ratios), :, :]>0.5)).nonzero()#((rpn_cls_prob_max<0.5) & (rpn_cls_prob_max>0.3)).nonzero()
            # fg_fea_ori = torch.mean(rpn_conv1_ori[fg_idx_ori[:,0].long(),:,fg_idx_ori[:,2].long(),fg_idx_ori[:,3].long()],dim=0).unsqueeze(dim=0)
            # bg_fea_ori = torch.mean(rpn_conv1_ori[bg_idx_ori[:, 0].long(), :, bg_idx_ori[:, 2].long(), bg_idx_ori[:, 3].long()],dim=0).unsqueeze(dim=0)
            # fg_fea_new = torch.mean(rpn_conv1[fg_idx_ori[:, 0].long(), :, fg_idx_ori[:, 2].long(), fg_idx_ori[:, 3].long()],dim=0).unsqueeze(dim=0)
            # bg_fea_new = torch.mean(rpn_conv1[bg_idx_ori[:, 0].long(), :, bg_idx_ori[:, 2].long(), bg_idx_ori[:, 3].long()],dim=0).unsqueeze(dim=0)
            # fg_fea_cat = torch.cat((fg_fea_ori, fg_fea_new), dim=1).reshape(fg_fea_ori.shape[0],-1,fg_fea_ori.shape[1])
            # bg_fea_cat = torch.cat((bg_fea_ori, bg_fea_new), dim=1).reshape(bg_fea_ori.shape[0],-1,bg_fea_ori.shape[1])
            #
            # features = torch.cat((fg_fea_cat,bg_fea_cat),dim=0)
            #
            # labels = torch.cat((torch.ones(fg_fea_ori.shape[0]).long().cuda(),torch.zeros(fg_fea_ori.shape[0]).long().cuda()),dim=0)
            #
            ...
            # features = features.div(torch.norm(features, p=2, dim=2, keepdim=True))
        ...
        if self.training:
            ...
            if fasterRCNN_org:
                ...
                self.rpn_conv1_distil_loss = l1_loss_fn(rpn_conv1,rpn_conv1_ori)#l1_loss_fn(rpn_conv1_norm, rpn_conv1_org_norm)
                self.rpn_conv1_distil_loss = rpn_output_loss#torch.Tensor([0]).cuda()#rpn_output_loss + cts_loss#
                #######################################################################################

                self.rpn_conv1_distil_loss = torch.Tensor([0]).cuda()
...
class _RPN_distil_residual(nn.Module):
    ...
    def forward(self, base_feat, im_info, gt_boxes, num_boxes, fasterRCNN_org=None, fasterRCNN_residual=None,base_feat_org=None,base_feat_residual=None,base_feat_inc=None):
        ...
        if base_feat_org is not None:
            ...
            # base_feat_high = maxpool(base_feat_high)
            ...
            # base_feat_org_high = maxpool(base_feat_org_high)
            ...
            high_org_norm = F.normalize(high_org, dim=1)#(1,512,num_point)
