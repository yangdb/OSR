...
class ISDALoss(nn.Module):
    ...
    def isda_aug(self, fc, features, y, labels, cv_matrix, ratio, kg=None, out_new=None, feature_mean=None):
        ...
        if isinstance(fc, tuple):
            ...
            weight_m = torch.cat((weight_m_1, weight_m_2), dim=0)
        else:
            if cfg.TRAIN.excls:
    ...
    #
    #     return loss, y
    def forward(self, features, fc, x, target_x, ratio, bg=False, kg=None, out_new=None, feature_mean=None):
        ...
        if torch.sum(self.estimator.Amount)>200:
            try:
                isda_aug_y = self.isda_aug(fc, features, y, target_x, self.estimator.CoVariance.detach(), ratio, kg=kg, out_new=out_new, feature_mean=feature_mean)
                loss = self.cross_entropy(isda_aug_y, target_x)
